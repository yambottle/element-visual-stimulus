classdef Monet < stims.core.Visual & stimulus.Monet
    
    methods(Access=protected)
        
        function d = degPerPix(self,constants)
            % assume isometric pixelsmetho
            rect = self.rect;
            if isempty(rect)
                rect = [0 0 1024 600];
            end
            d = 180/pi*constants.monitor_size*2.54/norm(rect(3:4))/constants.monitor_distance;
        end
        ...
        function prepare(self)
            if ~isfield(self.conditions, 'movie')
                ...
                for iCond=1:length(self.conditions)
                    ...
                    lookup = vis.MonetLookup;
                    [movie, key] = ...
                        lookup.lookup(cond, self.degPerPix(self.constants)*rect(3:4), ...
